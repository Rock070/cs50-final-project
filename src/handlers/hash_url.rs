use crate::{application::AppState, domain::HashUrlBody, entity::{urls, users}};
...
pub async fn hash_url(
    ...
    if let Some(authorization_header) = authorization {
        ...
        let claim = match state.jwt_handler.clone().decode_token(&state.application, token) {
            Ok(claim) => claim,
            Err(err) => {
        ...
        if !user_name.is_empty() {
            ...
            let urls_column = urls::Entity::find()
                .filter(
                    urls::Column::Url.eq(url_body.url.clone())
        ...
        let urls_column = urls::Entity::find()
            .filter(
                urls::Column::Url.eq(&url_body.url)
    ...
    let user_id = if user_id.is_empty() { None } else { Some(Uuid::parse_str(&user_id).unwrap()) };

    let url = urls::ActiveModel {
        id: ActiveValue::Set(Uuid::new_v4()),
        url: ActiveValue::Set(url_body.url),
        user_id: if user_id.is_some() { ActiveValue::Set(user_id) } else { ActiveValue::NotSet },
